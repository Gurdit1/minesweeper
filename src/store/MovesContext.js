...
import {
  ...
  getAdjacentTiles,
} from "../helpers/AdjacentTiles.js";

export const MovesContext = createContext();

function getAdjacentEmptyTiles(move, minefield) {
  return getAdjacentTiles(move.rowIndex, move.columnIndex, minefield).filter(
    (adjacentTile) =>
      calculateNumAdjacentMines(
...
function getAdjacentEmptyHiddenTiles(move, minefield) {
  return getAdjacentEmptyTiles(move, minefield).filter(
    (adjacentTile) => adjacentTile.tile.state === TILE_STATE.Hidden,
  );
}

export function MovesContextProvider({ children }) {
  const [startingMinefield, setStartingMinefield] =
    useState(generateMinefield());
  ...
  function computeCurrentMinefieldState() {
    ...
    moves.forEach((move) => {
      minefield[move.rowIndex][move.columnIndex] = {
        ...minefield[move.rowIndex][move.columnIndex],
        state: move.userState,
      };

      const selectedTile = minefield[move.rowIndex][move.columnIndex];
      if (
        move.userState === TILE_STATE.Revealed &&
        selectedTile.type === TILE_TYPE.Safe
      ) {
        if (
          calculateNumAdjacentMines(
            move.rowIndex,
          ...
          var outerTiles = [];
          var disoveredEmptyTiles = getAdjacentEmptyHiddenTiles(
            move,
            minefield,
          );
          ...
          while (disoveredEmptyTiles.length > 0) {
            for (const disoveredEmptyTile of disoveredEmptyTiles) {
              minefield[disoveredEmptyTile.rowIndex][
                disoveredEmptyTile.columnIndex
              ] = {
                ...minefield[disoveredEmptyTile.rowIndex][
                  disoveredEmptyTile.columnIndex
                ],
            ...
            for (const revealedTile of revealedTiles) {
              const newDiscoveredEmptyTiles = getAdjacentEmptyHiddenTiles(
                revealedTile,
                minefield,
              );
              disoveredEmptyTiles = [
                ...disoveredEmptyTiles,
                ...newDiscoveredEmptyTiles,
              ];
          ...
          for (const outerTile of outerTiles) {
            const remainingTiles = getAdjacentTiles(
              outerTile.rowIndex,
              outerTile.columnIndex,
              minefield,
            );
            for (const remainingTile of remainingTiles) {
              minefield[remainingTile.rowIndex][remainingTile.columnIndex] = {
                ...minefield[remainingTile.rowIndex][remainingTile.columnIndex],
                state: TILE_STATE.Revealed,
              };
            }
  ...
  function leftClickTile(rowIndex, columnIndex) {
    ...
    if (getGameState(minefield) !== GAME_STATE.Playing) {
      return;
    }
  ...
  return (
    <MovesContext.Provider value={ctxValue}>{children}</MovesContext.Provider>
  );
}
